// CHALLENGE 1: REVERSE A STRING
// Return a string in reverse
// ex. reverseString('hello') === 'olleh'

function reverseString(str) {
  return str
    .split("")
    .reverse()
    .join("");
}

// CHALLENGE 2: VALIDATE A PALINDROME
// Return true if palindrome and false if not
// ex. isPalindrome('racecar') === 'true', isPalindrome('hello') == false

function isPalindrome(str) {
  return str === reverseString(str) ? true : false;
}

// CHALLENGE 3: REVERSE AN INTEGER
// Return an integer in reverse
// ex. reverseInt(521) === 125

function reverseInt(int) {
  /* const intStr = int.toString();
  return parseInt(reverseString(intStr), 10); */
  let revInt = 0;
  while (int > 0) {
    revInt = revInt * 10 + (int % 10);
    int = parseInt(int / 10, 10);
    // console.log(int);
  }
  return revInt;
}

// CHALLENGE 4: CAPITALIZE LETTERS
// Return a string with the first letter of every word capitalized
// ex. capitalizeLetters('i love javascript') === 'I Love Javascript'
function capitalizeLetters(str) {
  const strArr = str.split(" ");
  return strArr.map(firstLetterToUppercase).join(" ");
}
// First Letter in word to Uppercase
function firstLetterToUppercase(word) {
  let capWord = word.split("");
  // console.log(word);
  capWord[0] = capWord[0].toUpperCase();
  // console.log(typeof capWord[0]);
  return capWord.join("");
}

// CHALLENGE 5: MAX CHARACTER
// Return the character that is most common in a string
// ex. maxCharacter('javascript') == 'a'
function maxCharacter(str) {
  const strArr = str.split("");
  const charSet = {};
  let maxChar = "",
    maxCount = 0;
  strArr.forEach(char => {
    if (charSet[char]) {
      charSet[char]++;
    } else {
      charSet[char] = 1;
    }
  });
  for (let char in charSet) {
    if (charSet[char] > maxCount) {
      maxChar = char;
      maxCount = charSet[char];
    }
  }
  // console.log(charSet);
  return maxChar;
}

// CHALLENGE 6: FIZZBUZZ
// Write a program that prints all the numbers from 1 to 100.
// For multiples of 3, instead of the number, print "Fizz", for multiples of 5 print "Buzz".
// For numbers which are multiples of both 3 and 5, print "FizzBuzz".
function fizzBuzz() {
  for (let i = 1; i <= 100; i++) {
    if (i % 3 === 0 && i % 5 === 0) {
      console.log("FizzBuzz");
    } else if (i % 3 === 0) {
      console.log("Fizz");
    } else if (i % 5 === 0) {
      console.log("Buzz");
    } else {
      console.log(i);
    }
  }
}

// Call Function
// const output = reverseString("world");
// const output = isPalindrome("madam");
// const output = reverseInt(123452134);
// const output = capitalizeLetters("i love javascript");
// const output = maxCharacter("javascript");
// const output = maxCharacter("josline");

// console.log(output);
fizzBuzz();
